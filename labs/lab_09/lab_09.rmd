...
### Chart URL: https://datawrapper.dwcdn.net/wDLpF/1/

### The story here is how voting habits in 2022 shifted back to closer normal after the COVID-19 pandemic, but that they are not fully back to what they were.
...
### Start by looking at a line chart.
...
### I picked the early voting turnout by the 65+ age group because I was curiousb about if they came to the polls early on in early voting because they are retired and in theory have more free time. I think this would have been more interesting to look at if I had lines for early voting turnout for other age groups as well or at least also included the eraly voting turnout by the 18-24 age group. In my code, I created a new column for total because I was more interested in learning about the state as a whole than particular counties.

### Chart URL: https://datawrapper.dwcdn.net/xYAup/1/

```{r}

ev_totals_65_up <- read_csv("data/ev_totals_65_up.csv")

ev_totals_65_up_total <- ev_totals_65_up |>
...
write_csv(ev_totals_65_up_total, "data/ev_totals_65_up_total.csv")
...
### 2 inner joins: population info+dems, population info+reps
...
### Act Blue Chart URL: https://datawrapper.dwcdn.net/Uy7OD/3/

### Win Red Chart URL: https://datawrapper.dwcdn.net/mDM0M/2/

### The story idea that most jumps out is that the highest number of donations for both come from the same area. This is despite the Act Blue donation rate being much higher in that area. I would also be interested in learning about the bluer zip codes in the Win Red chart that are further out from Montgomery County. One story idea that could be interesting is looking to Act Blue donators in the Potomac, Bethesda, and Chevy Chase zip codes that could be neighbors with Win Red donators. It could also be cool to do stories on the areas with high donation rates that do not have high total donation amounts.
...
md_zip_data <- get_acs(geography="zcta", 
                              variables = c(totalpopulation="B01003_001"),
                              state='MD',
                              year=2019)

md_zip_data_new <- md_zip_data |>
    rename(zip = GEOID, populationtotal=estimate) 


md_actblue_zip_totals$zip <- as.character(md_actblue_zip_totals$zip)
md_zip_data_new$zip <- as.character(md_zip_data_new$zip)
md_winred_zip_totals$zip <- as.character(md_winred_zip_totals$zip)

zip_dem <- inner_join(md_actblue_zip_totals, md_zip_data_new, by = "zip") |>
  mutate(donation_rate_per_1000 = (donations / populationtotal)*1000)


zip_rep <- inner_join(md_winred_zip_totals, md_zip_data_new, by = "zip") |>
    mutate(donation_rate_per_1000 = (donations / populationtotal)*1000)

write_csv(zip_dem, "data/zip_dem.csv")


write.csv(zip_rep, "data/zip_rep.csv")
```
