...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The dataframe is 999 of Cardin's press releases with the date, title, url and full text.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It only shows me a list of unique results in the list. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** I get a list of each words from the text we have selected. I get a one column dataframe with all of the words (not unique words) from all of the press releases. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** No. It gives us boring words.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes it is more interesting than before.
...
**Task** Run the following code and describe the results. **Answer** Some words were more repeated in 2024. Baltimore was the number one word in 2024. This probably had something to do with the Key Bridge. We also don't have all of the 2024 values yet so all those numbers could change.
...
**Task** Run the following code and describe the results. **Answer** I have the top 10 two-word phrases. The first one is related to funding. A lot of them are related to places like Baltimore City, Prince George's County, and the Chesapeake Bay. COVID-19 is also prevalent. 
...
**Task** Run the following code and describe the results. **Answer** We get a list of the top 10 word phrases for 2024. Health care is ranked highly again. A lot of these are pretty similar, just in a different order. COVID-19 is no longer in the list. 
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** Stem words does not group similar words. Lemma does so it is probably more useful. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The lemma one is more useful because it groups similar words. This means funds and funding aren't both in the top results. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** I have a list of the top words and their lexicon and sentiments. Rail being negative seems wrong. Critical also being negative seems wrong. It could just mean the same thing as important. 
