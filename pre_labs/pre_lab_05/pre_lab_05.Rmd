...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Yes, they do. Some of the columns are in different orders, but they have the same names.
...
**Task** Load the Maryland 2020 county population data **Answer** Not sure what I am supposed to answer here. 
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** We should only get 24 rows.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** The output message says "Joining with `by = join_by(COUNTY)`" It is telling us that the data is joined through the county column. Not encouraged to do this. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** 1. Howard County. 2. Baltimore City. These results are different as the two answers are not aligned. The pct_dems looks less correlated with pct_black_18_plus. The Asian population is more clustered in Maryland than the Black community and is also a smaller population. 
...
### zipcodes are not numbers. always convert them to chr.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** chr (character)
