...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** R is telling us that there are 1,640 rows with problems where we expect a character column (true/false) but actually get numbers in response. 
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No, it does not show any parsing errors. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** The district field is a number column. The precinct field is a character column. The mail column is a number column.
...
### .default says leave everything else as is
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** I think many of these could be worthy of exploring. It would be interesting to look into how many felons are not able to vote and also the reasoning for the military category. I'd also want to talk to people who haven't voted for awhile and who have requested to be removed from the voter roll. I am also interested in learning what other information the county is waiting on to confirm these voters. 
...
**Answer** There are 14 rows in this demonstratin data set. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** The amount column is the character data type.
...
**Answer**Numbers in the front of column names, upper case and lowercase column names, numbers being stored in chr columns. Duplicated rows
...
**Answer**It adds an x before so it is "x1_linenumber." This is because it is bad practice for column names to begin with numbers. It also standardizes the names of columns so that they are all lowercase. It replaces spaces with underscores. 
...
We got an error when we tried to add up (sum) a character column.

**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** It gives us an error saying that we cannot sum() because the data type is character. 
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** The new data type is "dbl."
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer**I found two rows. One is a duplicate of the other. The donor name is Derrick Hamilton. 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** This gives me all the unique records but only one of each. The dataframe now has 13 rows. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer**The zipcodes are standardized and are each five digits now. 
...
**Answer** I still see two misspellings of Conowingo.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** I still see 1 misspelling of Conowingo.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** There are zero misspellings left. 
