...
**Answer** The code is choosing to display specific columns and creating a new column that shows us the percentage of people who voted on Election Day. 
...
**Answer** The results are now displayed in descending order based on candidates who had the greatest percentage of votes actually voted on Election Day. 
...
**Answer** I think both orderings are useful. They both allow you to see the data in order and notice that Democratic candidates seem to have a lower percentage of Election Day voting. Ascending order does allow you to notice this a little bit quicker. 
...
**Answer**str_to_lower does the opposite of str_to_upper. It changes a column of characters to all lowercase. str_to_title changes the column of characters to all title case. 
...
**Answer**It creates a column called amount_category and then uses case_when to handle the value varying on a case to case basis. For example, when the value is great than or equal to \$100 and less than \$500 then it gets "Between \$100 and \$499" in its column.
...
**Answer** It is important that we made a new variable to hold the result because it allows us to view the new data table easily and return to it quickly. We can also perform functions on the newly made and saved variable. 
...
**Answer:** The date is written in an actual date format instead of in a character format. 
...
**Answer**The values in the new 'month' column say the first date of the month but encapsulate all dates in a month. All values in a month were grouped. THe data is showing the total amount of expenditures for each month. 