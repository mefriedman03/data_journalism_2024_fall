...
### always use janitor when you are scraping
```{r}
#| output: false
...
### view-source:https://elections.maryland.gov/elections/2024/general_Results/gen_detail_results_2024_1_1.html
...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** This shows us just the first table. I notice the bottom rows have the percentage and a number in every column and that the column names are long. 
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** I think the numbers are referring to the order of the candidates who got the most votes.

### slice is always for rows
### we are subsituting commas with nothing through gsub
```{r}
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** The output suggests that there are 55. 
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** Long list of text characters. Some are not committees or task forces. 
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** I now have a dataframe that includes the committees with URLs and the not committees without URLs.
