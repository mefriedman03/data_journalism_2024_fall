...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **Answer** I'm not sure what I am supposed to answer here!
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** The names are written in many different ways. This doesn't let them group correctly. We need to clean up the names. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** I see that "W. Millar & Co."	is now "W. MILLAR & CO." and "W.Millar & Company Catering" is now "W. MILLAR & CO. CATERING." It standardizes to "WASHINGTON HILTON" and "WALMART."
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No, I don't think it changes it substantially. 
