...
```{r}
voters_by_county <- read_csv("tabula-Eligible_Active_Voters_by_County.csv")
...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** I see that the column names are numbers which means that there are not headers and that there are a lot of NAs.

```{r}
benzie_2024 <- read_csv("benzie_one.csv")
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Most of the NAs are gone and the numbers are no longer the column names, but there are now place holder column names. There are also still some columns that are basically just filled with NAs.

```{r}
benzie_2024 <- read_csv("benzie_one.csv", col_names = FALSE)
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** There are now proper column names and there is a party and an office column. There are still some column names unlabeled and some empty looking rows and columns. 
